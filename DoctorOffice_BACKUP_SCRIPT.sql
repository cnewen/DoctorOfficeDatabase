-- phpMyAdmin SQL Dump
-- version 4.9.1
-- https://www.phpmyadmin.net/
--
-- Host: localhost
-- Generation Time: Nov 26, 2019 at 03:01 AM
-- Server version: 10.4.8-MariaDB
-- PHP Version: 7.1.32

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
SET AUTOCOMMIT = 0;
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Database: `DoctorOffice`
--

USE `DoctorOffice`;

-- DOCTOR TABLE BACKUP
DROP TABLE IF EXISTS DOCTOR_BACKUP;
CREATE TABLE DOCTOR_BACKUP 
SELECT *
FROM DOCTOR;

-- DOCTORSPECIALTY TABLE BACKUP
DROP TABLE IF EXISTS DOCTORSPECIALTY_BACKUP;
CREATE TABLE DOCTORSPECIALTY_BACKUP 
SELECT *
FROM DOCTORSPECIALTY;

-- PATIENT TABLE BACKUP
DROP TABLE IF EXISTS PATIENT_BACKUP;
CREATE TABLE PATIENT_BACKUP 
SELECT *
FROM PATIENT;

-- PATIENTVISIT TABLE BACKUP
DROP TABLE IF EXISTS PATIENTVISIT_BACKUP;
CREATE TABLE PATIENTVISIT_BACKUP 
SELECT *
FROM PATIENTVISIT;

-- PERSON TABLE BACKUP
DROP TABLE IF EXISTS PERSON_BACKUP;
CREATE TABLE PERSON_BACKUP 
SELECT *
FROM PERSON;

-- PRESCRIPTION TABLE BACKUP
DROP TABLE IF EXISTS PRESCRIPTION_BACKUP;
CREATE TABLE PRESCRIPTION_BACKUP 
SELECT *
FROM PRESCRIPTION;

-- PVISITPRESCRIPTION TABLE BACKUP
DROP TABLE IF EXISTS PVISITPRESCRIPTION_BACKUP;
CREATE TABLE PVISITPRESCRIPTION_BACKUP 
SELECT *
FROM PVISITPRESCRIPTION;

-- PVISITTEST TABLE BACKUP
DROP TABLE IF EXISTS PVISITTEST_BACKUP;
CREATE TABLE PVISITTEST_BACKUP 
SELECT *
FROM PVISITTEST;

-- SPECIALTY TABLE BACKUP
DROP TABLE IF EXISTS SPECIALTY_BACKUP;
CREATE TABLE SPECIALTY_BACKUP 
SELECT *
FROM SPECIALTY;

-- TEST TABLE BACKUP
DROP TABLE IF EXISTS TEST_BACKUP;
CREATE TABLE TEST_BACKUP 
SELECT *
FROM TEST;

-- --------------------------------------------------------


/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;